# CData Dremio ARP Connector for Amazon Athena

The Amazon Athena connector allows Dremio to connect to and query data in Amazon Athena. This can then allow you to build custom reports, dashboards, or even just ad-hoc SQL via your client tool of choice. Note that it does require a third-party JDBC driver that is not free, but does allow a free trial.
...
3. Download and install the [Amazon Athena JDBC Driver from CData](https://www.cdata.com/drivers/athena/download/jdbc)* and copy the JAR file to the /jars/3rdparty/ directory of your Dremio installation. For example:
